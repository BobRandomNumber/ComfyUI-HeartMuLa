...
class HeartMuLa(PreTrainedModel):
    ...
    def generate_backbone_step(
        ...
        if continuous_segments is not None:
            ...
            if uncond_mask is not None:
                ...
                ).to(dtype=continuous_segments.dtype)
    ...
    def generate_decoder_step(
        ...
        for i in range(1, self.config.audio_num_codebooks):
            ...
            decoder_h = self.decoder(
                self.projection(curr_h).to(dtype=self.text_embeddings.weight.dtype), input_pos=curr_pos, mask=curr_decoder_mask
            )
    ...
    def _embed_audio(self, codebook: int, tokens: torch.Tensor) -> torch.Tensor:
        return self.audio_embeddings(tokens + codebook * self.config.audio_vocab_size).to(dtype=self.text_embeddings.weight.dtype)

    def _embed_tokens(
        self, tokens: torch.Tensor, uncond_mask: torch.Tensor | None
    ) -> torch.Tensor:
        B, S, _ = tokens.size()
        target_dtype = self.text_embeddings.weight.dtype
        text_embeds = self.text_embeddings(tokens[:, :, -1]).to(dtype=target_dtype)
